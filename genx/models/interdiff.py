...
LayerParameters={'sigmai':0.0,'sigmar':0.0,'reldens':1.0,'d':0.0,'n':1.0+0.0j}
...
def Specular(TwoThetaQz,sample,instrument):
    ...
    if restype==2:
            (TwoThetaQz,weight)=ResolutionVector(TwoThetaQz[:],instrument.getRes(),instrument.getRespoints(),range=instrument.getResintrange())
    ...
    R=Paratt.Refl(theta,lamda,(n-1.0)*reldens+1.0,d,sigma)*instrument.getI0()
...
def OffSpecularMingInterdiff(TwoThetaQz,ThetaQx,sample,instrument):
    ...
    #print eta_z
    if __offspec__:
        (I,alpha,omega)=offspec2_weave.DWBA_Interdiff(qx,qz,lamda,n,z,sigmar,sigmai,eta,h,eta_z,d)
    else:
        I=ones(len(qx*qz))
...
(Instrument,Layer,Stack,Sample)=Refl.MakeClasses(InstrumentParameters,LayerParameters,StackParameters,SampleParameters,SimulationFunctions,ModelID)
