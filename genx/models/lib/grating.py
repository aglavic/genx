...
def coherent_nonspec_refl(k_in, k_out, lamda, vf_layers, v_mean, d, kin=False, spec_refl=False):
    ...
    w = vf_layers * (T_2[:, :-1] * T_1[:, :-1] * ftz_layer(q0, d) +
...
def coherent_refl(k_in, k_out, lamda, vf, v_mean, d, area_uc, kin=False):
    ...
    R = np.where(np.bitwise_and(is_spec(k_in, k_out), not kin),
                 np.abs(r)**2,
                 1./np.abs(k_in[2])/np.abs(k_out[2])/4/area_uc**2*np.abs(T_12) ** 2
                 )
    return R


def is_spec(k_in, k_out):
    return np.bitwise_and(np.isclose(k_in[0], k_out[0]),
                          np.bitwise_and(np.isclose(k_in[1], 0), np.isclose(k_out[1], 0)))
...
if __name__ == '__main__':
    ...
    if False:
        ...
        # print v_lay
        d = array([0, 1, 0])
        vf_list = [mult(v_lay, ft_layer(a*b)), mult(v_lay, ft_circle(radius=a/4.)), mult(0, ft_layer(a*b))]
    ...
    if True:
        ...
        # print v_lay
        d = array([2, 500, 50, 1])
        vf_list = [mult(v_lay, ft_layer(a*b)), mult(v_lay, ft_circle(radius=a / 4.*1.5)), mult(v_lay, ft_circle(radius=a / 4.*1.5)), mult(0, ft_layer(a*b))]
        vf_func = assemble_vf_func(vf_list)
