...
class Parameters(H5Savable):
    ...
    def read_h5group(self, group):
        ...
        self.data = [
            [c0.decode("utf-8"), float(c1), bool(c2), float(c3), float(c4), c5.decode("utf-8")]
            for (c0, c1, c2, c3, c4, c5) in zip(
                group["data col 0"][()],
                group["data col 1"][()],
                group["data col 2"][()],
                group["data col 3"][()],
    ...
    def get_fit_pars(self):
        ...
        row_nmb = [nmb for nmb in rows if self.data[nmb][2] and not self.data[nmb][0] == ""]
        ...
        for line in lines[:-1]:
            ...
            if line[0] != "#" and line[0] != "\n":
                ...
                try:
                    ...
                    fitted = line_strs[2].strip() == "True" or line_strs[2].strip() == "1"
    ...
    def _repr_html_(self):
        ...
        output += (
            "           <tr><th>No.</th><th>"
            + "</th><th>".join(["%s" % label for label in self.data_labels])
            + "</th></tr>\n"
        )
    ...
    def _repr_ipyw_(self):
        ...
        header = ipw.HBox(
            [
                ...
                for txt in [("Parameter", "35%"), ("Value", "20%"), ("fit", "5%"), ("min", "20%"), ("max", "20%")]
            ]
        )
        vlist.append(header)
        for par in self:
...
class ConnectedParameter:
    ...
    def set_name(self, value, model):
        ...
        par_value = eval("self._model.script_module." + value.replace(".set", ".get") + "()", globals(), locals())
        self.data[0] = value
        self.value = par_value
        self.min = 0.25 * par_value
        self.max = 4.0 * par_value
    ...
    def _repr_html_(self):
        ...
        output += (
            "           <tr><th>No.</th><th>"
            + "</th><th>".join(["%s" % label for label in self._parent.data_labels])
            + "</th></tr>\n"
        )
