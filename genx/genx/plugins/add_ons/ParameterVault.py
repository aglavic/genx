...
class Plugin(framework.Template):
    ...
    def create_toolbar(self):
        ...
        self.parameter_panel.Bind(wx.EVT_BUTTON, self.parameter_list.DeleteItem,
                                  self.bitmap_button_delete)
        self.parameter_panel.Bind(wx.EVT_BUTTON, self.parameter_list.ApplyParameters,
                                  self.bitmap_button_apply)
        self.parameter_panel.Bind(wx.EVT_BUTTON, self.parameter_list.TogglePlot,
    ...
    def create_parameter_list(self):
        ...
        psizer.Add(wx.StaticText(ppanel, label='P1 index:'),
                   proportion=0, flag=wx.EXPAND, border=2)
        ...
        psizer.Add(wx.StaticText(ppanel, label='P2 index:'),
                   proportion=0, flag=wx.EXPAND, border=2)
        ...
        self.parameter_panel.Bind(wx.EVT_SPINCTRL,
                                  self.parameter_list.UpdateParams, self.P1_index)
        self.parameter_panel.Bind(wx.EVT_SPINCTRL,
                                  self.parameter_list.UpdateParams, self.P2_index)
    ...
    def OnRecalcAll(self, event):
        ...
        for i in range(len(pl.parameter_list)):
            ...
            pl.parameter_list[i]=[model.fom or 0., True, params, data, slds]
    ...
    def OnAutoStore(self, event):
        ...
        if self.mb_auto_store.IsChecked():
            for i, (ignore, ignore, params, ignore, ignore) in \
                    enumerate(self.parameter_list.parameter_list):
                p.data=params
...
class ParameterList(wx.ListCtrl, ListCtrlAutoWidthMixin):
    ...
    def AddItem(self, ignore, simulate=True):
        ...
        params=deepcopy(model.parameters.data)
        data=[di.copy() for di in model.data]
        self.parameter_list.append([model.fom or 0., True, params, data, slds])
        self.SetItemCount(len(self.parameter_list))
        self.plugin.OnAutoStore(None)
        if simulate:
