...
def STFT(q, y, d=None, tau=1.):
    ...
    if d is None:
        return_d=True
        d=linspace(0, len(q)*2.*pi/(q.max()-q.min()), len(q))
    FT=zeros(len(d), dtype=complex)
    for i, di in enumerate(d):
        FT[i]=trapz(y*_G(y/2./pi-di, tau)*exp(1j*di*q), x=q)
    if return_d:
        return d, FT
    else:
        return FT
...
def _G(t, tau):
    return exp(-pi/2.*(t/tau)**2)

def MexicanHat(t):
    return (1.-t**2)*exp(-t**2/2.)

def Morlet(t, sigma=1.):
    return pi**(-0.25)*exp(-t**2/2.)*(exp(1j*sigma*t)-exp(-sigma**2/2.))
...
def derivative(x, y, N):
    ...
    if N==1:
        ...
        dydx[N:-N] = (dydx_csum[2*N-2:]-dydx_csum[:(2-2*N) or None])/2./(N-1)
        for i in range(2, N):
            dydx[i] = (dydx_csum[2*i-2]-dydx_csum[0])/2./(i-1)
            dydx[-i] = (dydx_csum[2-2*i]-dydx_csum[-1])/2./(i-1)
...
def average(y, N=20):
    ...
    for i in range(1, N//2+1):
        avg[i] = csum[2*i]/(2.*i+1.)
        avg[-i-1] = (csum[-1]-csum[-2*i-2])/(2.*i+1.)
    return avg
