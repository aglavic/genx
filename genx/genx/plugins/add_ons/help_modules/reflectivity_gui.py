    ...
    def update_grid(self, vals, states):
        ...
        for name, value_info in self.instance._parameter_info().items():
            ...
            if states[name] and self.par_editable[name] != 3 and self.par_editable[name] != states[name]:
                ...
                # if the state of a parameter is changed to fit or set in grid, update the grid line
...
class SamplePanel(wx.Panel):
    ...
    def InsertStack(self, evt):
        ...
        pars = ["Name"]
        vals["Name"] = "name"
        dlg = ValidateDialog(self, pars, vals, validators, title="Give Stack Name")
        ...
        if sl:
            ...
            self.plugin.ShowWarningDialog("Can not insert a stack at the" " current position.")
...
class DataParameterPanel(wx.Panel):
    ...
    def do_toolbar(self):
        ...
        button_images = [
            wx.Bitmap(images.add.GetImage().Scale(tb_bmp_size, tb_bmp_size)),
...
class EditCustomParameters(wx.Dialog):
    ...
    def __init__(self, parent, model, lines):
        ...
        name_ctrl_sizer.Add(self.error_ctrl, (1, 2), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, border=5)
        self.add_button = wx.Button(self, id=wx.ID_ANY, label="Add")
        name_ctrl_sizer.Add(self.add_button, (1, 3), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, border=5)
    ...
    def OnAdd(self, event):
        ...
        if sigma.strip() == "":
            line = "%s.new_var('%s', %s)" % (self.var_name, self.name_ctrl.GetValue(), self.value_ctrl.GetValue())
        else:
...
class SimulationExpressionDialog(wx.Dialog):
    ...
    def __init__(self, parent, model, instruments, sim_func, arguments, inst_name, data_index):
        ...
        self.sim_choice = wx.Choice(self, -1, choices=self.available_sim_funcs)
        ...
        gbs.Add(self.sim_choice, (1, 0), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, border=5)

        # Instrument choice control
        self.inst_choice = wx.Choice(self, -1, choices=list(self.instruments.keys()))
        ...
        gbs.Add(self.inst_choice, (1, 1), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, border=5)
...
class ParameterExpressionDialog(wx.Dialog):
    ...
    def __init__(self, parent, model, expression=None, sim_func=None):
        ...
        self.expression_ctrl = ParameterExpressionCombo(par_dict, sim_func, self, -1, exp_right, size=(300, -1))
