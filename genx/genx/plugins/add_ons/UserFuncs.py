...
from .help_modules.custom_dialog import ValueValidator, ValidateDialog


DEFAULT_VALUE = {int: 0, float: 0., str: ''}
...
class Plugin(framework.Template):
    ...
    def exec_args_function(self, fname):
        ...
        dia_vals = dict([(ai, defaults[ai]) if defaults[ai] is not inspect.Parameter.empty
                        else (ai, DEFAULT_VALUE[annots[ai]])
                        for ai in dia_args])
