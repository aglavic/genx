...
class Plugin(framework.Template):
    ...
    def _init_refplugin(self):
        ...
        if "Reflectivity" in ph.loaded_plugins:
            ...
            # connect to the reflectivity plugin for layer creation
            self._refplugin = SimplePluginInterface(ph.loaded_plugins["SimpleReflectivity"])
        else:
...
class MaterialsList(wx.ListCtrl, ListCtrlAutoWidthMixin):
    ...
    def __init__(self, parent: wx.Panel, materials_list: MaterialsDatabase):
        ...
        for col, (text, width) in enumerate(
            [("Chemical Formula", 80), ("n [10⁻⁶Å⁻²]", 60), ("kα [rₑ/Å⁻³]", 60), ("FU/Å³", 60), ("g/cm³", 60)]
        ):
...
class MaterialDialog(wx.Dialog):
    ...
    def _create_entries(self):
        ...
        table.Add(wx.StaticText(self, label="Extracted Elements:"), (1, 0), span=(1, 3), flag=wx.ALIGN_CENTER)
        self.formula_display = wx.TextCtrl(self, size=(150, 100), style=wx.TE_MULTILINE | wx.TE_READONLY)
        #        self.formula_display.Enable(False)
        table.Add(self.formula_display, (2, 0), span=(1, 3), flag=wx.EXPAND)

        table.Add(wx.StaticText(self, label="1. Unit Cell Parameters:"), (3, 0), span=(1, 3), flag=wx.ALIGN_CENTER)
        ...
        table.Add(wx.StaticText(self, label="2. Physical Parameter:"), (12, 0), span=(1, 3), flag=wx.ALIGN_CENTER)
    ...
    def OnUnitCellChanged(self, event):
        ...
        if params[3] == 90 and params[4] == 90 and params[5] == 90:
            ...
            alpha = params[3] * pi / 180.0
            beta = params[4] * pi / 180.0
            gamma = params[5] * pi / 180.0
    ...
    def OnCODQuery(self, event):
        db = pymysql.connect(host="sql.crystallography.net", user="cod_reader", password=None, database="cod")
        c = db.cursor()
        formula = self.format_formula()
        c.execute(
        ...
        if len(res) > 0:
            ...
            for i, ri in enumerate(res):
                ...
                text = "%s\n%s\n%s (%s)" % tuple(ri[12:16])
                items.append(f"{i+1}: {name} ({sgs}) |" f" Density: {dens:.3g} g/cm³ |" f" UC Volume: {V}\n{text}")
            dia = SCDialog(
                self,
                "Several entries have been found, please select appropriate:",
                "Select correct database entry",
    ...
    def analyze_cif(self, txt):
        cell_params = [1.0, 1.0, 1.0, 90.0, 90.0, 90.0, 1.0]
        composition = ""
        ...
        for line in file_lines:
            ...
            if line.startswith("_chemical_formula_structural"):
                composition = (
                    line.strip().split(None, 1)[1].replace(")", "").replace("(", "").replace("'", "").replace('"', "")
                )
