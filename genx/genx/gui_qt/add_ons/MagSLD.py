...
A plugin to show a second y-axis in the SLD plot that gives the magnetization corresponding
...
class Plugin(framework.Template):
    def __init__(self, parent):
        ...
        self.action_second_axis = menu.addAction("Show second SLD axis")
        self.action_second_axis.setCheckable(True)
        self.action_second_axis.setChecked(True)
        self.action_second_axis.setToolTip("Show second SLD axis for magnetization")

        self.action_use_si = menu.addAction("Use SI units")
        self.action_use_si.setCheckable(True)
        self.action_use_si.setChecked(False)
        self.action_use_si.setToolTip("Use SI units of magnetization")

        self.action_second_axis.triggered.connect(self._OnSimulate)
        self.action_use_si.triggered.connect(self._OnSimulate)
    ...
    def _OnSimulate(self, event):
        ...
        for i, di in enumerate(model.data):
            if di.show and sld_items > i:
                ...
                if "z" not in slds or "SLD unit" not in slds:
                    continue
                z = slds["z"]
                unit = slds["SLD unit"]
                if unit in ["fm/\\AA^{3}", "\\AA^{-2}", "", "10^{-6}\\AA^{-2}"] and self.action_second_axis.isChecked():
                    for key, value in list(slds.items()):
                        if key == "mag" and value.sum() > 0.0:
                            msld = value
                            mag = msld * AAm2_to_emucc
                            com = (msld * z).sum() / msld.sum()
                            if self.action_use_si.isChecked():
        ...
        if (
            ...
            and self.action_second_axis.isChecked()
        ):
            ...
            sld_plot.ax.legend(loc="upper right", framealpha=0.5, fontsize="small", ncol=1)
        else:
            ax2.set_visible(False)
