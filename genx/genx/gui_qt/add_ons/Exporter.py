...
Currently, only BornAgain is supported to easy transfer from specular
...
class Plugin(framework.Template):
    def __init__(self, parent):
        ...
        self.action_export_ba = self.menu.addAction("BornAgain script...")
        self.action_export_ba.setToolTip("Export reflectometry layers to BornAgain python script.")
        self.action_export_ba.triggered.connect(self.OnExportBA)

        self.StatusMessage("Successfully loaded Exporter...")
    ...
    def OnExportBA(self):
        ...
        for stack in s.Stacks:
            ...
            for layer in stack.Layers:
                if layer in objects:
                    name = "# " + names[objects.index(layer)]
                else:
                    name = ""
                output += "           " + self._expand_layer(layer) + f",{name}\n"
            output += "         ],\n"
        output += "       ]\n"
