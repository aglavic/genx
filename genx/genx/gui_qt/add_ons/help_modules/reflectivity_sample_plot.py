...
class SamplePlotPanel(PlotPanel):
    ...
    def __init__(self, parent, plugin, color=None, dpi=None, **kwargs):
        PlotPanel.__init__(self, parent, color=color, dpi=dpi, config_class=SamplePlotConfig, **kwargs)
        self.plugin = plugin
    ...
    def Plot(self):
        ...
        if getattr(self.plugin, 'sim_returns_sld', False) and model._sim:
        ...
        if i > 0:
            ...
            QtCore.QTimer.singleShot(0, self.flush_plot)
            self.AutoScale()
    ...
    def OnContextMenu(self, event):
        ...
        legend_action = menu.addAction("Legend Outside")
        legend_action.setCheckable(True)
        legend_action.setChecked(self.opt.legend_outside)
        ...
        legend_action.triggered.connect(on_legend)

        coloring_action = menu.addAction("Color from Data")
        coloring_action.setCheckable(True)
        coloring_action.setChecked(self.opt.data_derived_color)

        def on_coloring():
            self.opt.data_derived_color = not self.opt.data_derived_color
            self.Plot()

        coloring_action.triggered.connect(on_coloring)

        single_action = menu.addAction("Single Sample Model")
        single_action.setCheckable(True)
        single_action.setChecked(self.opt.show_single_model)

        def on_single_model():
            self.opt.show_single_model = not self.opt.show_single_model
            self.Plot()

        single_action.triggered.connect(on_single_model)
