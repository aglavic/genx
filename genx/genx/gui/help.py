...
class PluginHelpDialog(wx.Frame):
    ...
    def find_modules(self, module):
        ...
        try:
            modules = [s[:-3] for s in os.listdir(mod.__path__[0]) if s[0] != "_" and s[-3:] == ".py"]
    ...
    def load_doc(self, module, sub_module=None):
        ...
        try:
            if sub_module is not None:
                mod = __import__("genx.%s.%s" % (module, sub_module), globals(), locals(), [""])
            else:
                mod = __import__("genx.%s" % module, globals(), locals(), [""])
