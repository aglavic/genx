...
from .custom_events import (delete_parameters, grid_change, inset_parameter, move_parameter, set_parameter_value,
                            skips_event, sort_and_group_parameters, value_change)
...
class ParameterDataTable(gridlib.GridTableBase):
    ...
    def SetValue(self, row, col, value):
        ...
        if row >= par_len:
            ...
            msg = gridlib.GridTableMessage(
                self, gridlib.GRIDTABLE_NOTIFY_ROWS_APPENDED, 1  # The table  # what we did to it  # how many
            )
    ...
    def AppendRows(self, num_rows=1):
        ...
        msg = gridlib.GridTableMessage(self, gridlib.GRIDTABLE_NOTIFY_ROWS_APPENDED, num_rows)
        self.GetView().ProcessTableMessage(msg)
        self.UpdateView()
        return True
    ...
    def SetParameters(self, pars, clear=True, permanent_change=True):
        ...
        if clear:
            ...
            if diff_rows < 0:
                ...
                # rows were added
                msg = gridlib.GridTableMessage(self, gridlib.GRIDTABLE_NOTIFY_ROWS_APPENDED, diff_rows)
                self.GetView().ProcessTableMessage(msg)
            msg = gridlib.GridTableMessage(self, gridlib.GRIDTABLE_REQUEST_VIEW_GET_VALUES)
            self.GetView().ProcessTableMessage(msg)
...
class SliderCellEditor(gridlib.GridCellEditor):
    ...
    def Create(self, parent, id, evtHandler):
        self._tc = ctrls.SliderControl(
            parent, id, value=self.value, max_value=self.max_value, min_value=self.min_value, font=parent.GetFont()
        )
    ...
    def SetSize(self, rect):
        ...
        """
        self._tc.SetSize(rect.x - 1, rect.y - 1, rect.width + 2, rect.height + 2, wx.SIZE_ALLOW_MINUS_ONE)
...
class ValueLimitCellEditor(gridlib.GridCellEditor):
    ...
    def Clone(self):
        ...
        Create a new object which is the copy of this one
        """
        return ValueLimitCellEditor(
            value=self.value, min_value=self.min_value, max_value=self.max_value, ticks=self.ticks, digits=self.digits
        )
...
class ValueCellRenderer(gridlib.GridCellRenderer):
    ...
    def Draw(self, grid, attr, dc, rect, row, col, isSelected):
        if grid.GetCellValue(row, col) != "":
            ...
            dc.DrawText(text, rect.x + x_options[halign], rect.y + y_options[valign])
...
class ParameterGrid(wx.Panel, Configurable):
    ...
    def do_toolbar(self):
        ...
        self.slider_tool = self.toolbar.AddCheckTool(
            newid,
            label="Show sliders",
            bitmap1=wx.Bitmap(img.slider.GetImage().Scale(tb_bmp_size, tb_bmp_size)),
    ...
    def _update_printer(self):
        ...
        for row in self.GetParameters().get_data():
            ...
            data.append([row[0], "%.5f" % row[1], "%d" % row[2], "%.5f" % row[3], "%.5f" % row[4], row[5]])
        self.prt.data = data
    ...
    def OnNewModel(self, evt):
        ...
        self.table.SetParameters(evt.GetModel().get_parameters(), permanent_change=False)
    ...
    def OnPopUpItemSelected(self, event):
        ...
        if len(lis) == 2:
            try:
                ...
                self.table.SetValue(self.CurSelection[0], 3, min(minval, maxval))
                self.table.SetValue(self.CurSelection[0], 4, max(minval, maxval))
