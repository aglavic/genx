...
from .model_actions import (ActionHistory, DeleteParams, ExchangeModel, InsertParam, ModelAction, MoveParam, NoOp,
                            SortAndGroupParams, UpdateColorCycle, UpdateDataPlotSettings, UpdateModelScript,
                            UpdateParams, UpdateParamValue, UpdateSolverOptoins)
...
class ModelController:
    ...
    def read_sequence(self, data_loader: DataLoaderTemplate, file_lists, name_by_file=False, callback=None):
        data = self.get_data()
    ...
    def CalcErrorBars(self):
        ...
        if self.optimizer.n_fom_evals == 0:
            raise ErrorBarError(
                "Can not find any stored evaluations of the model in the optimizer.\n"
    ...
    def save_gx(self, fname: str):
        ...
        self.model.save_addition(
            "optimizer", self.optimizer.pickle_string(clear_evals=not config.getboolean("solver", "save all evals"))
        )
