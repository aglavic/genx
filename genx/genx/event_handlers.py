...
def open_model(self, path):
    ...
    try:
        [p.ReadConfig() for p in get_pages(self)]
    except Exception as e:
        outp=StringIO()
        traceback.print_exc(200, outp)
        val=outp.getvalue()
...
def on_new_model(self, event):
    ...
    self.main_frame_menubar.mb_use_toggle_show.Check(
        self.config.getboolean('data handling', 'toggle show'))
    try:
        val=self.config.getboolean('parameter grid', 'auto sim')
    except GenxOptionError:
        iprint('Could not locate option parameters.auto sim')
        self.main_frame_menubar.mb_fit_autosim.Check(True)
    else:
        self.main_frame_menubar.mb_fit_autosim.Check(val)
...
def update_for_save(self):
    ...
    self.config.set('parameter grid', 'auto sim', self.main_frame_menubar.mb_fit_autosim.IsChecked())
...
def parameter_value_changed(self, event):
    ...
    if self.main_frame_menubar.mb_fit_autosim.IsChecked() and not self.flag_simulating:
        _thread.start_new_thread(simulation_loop, (self,))
...
def simulation_loop(self):
    ...
    while self.simulation_queue_counter>0:
        do_simulation(self, from_thread=True)
        time.sleep(0.1)
...
def simulate(self, event):
    ...
    self.flag_simulating=True
    do_simulation(self)
    set_possible_parameters_in_grid(self)
    self.flag_simulating=False
...
def calculate_error_bars(self, evt):
...
def on_optimizer_settings(self, event):
...
def on_data_loader_settings(self, event):
...
def quit(self, event):
    ...
    hsize, vsize=self.GetSize()
    self.config.default_set('gui', 'hsize', hsize)
    self.config.default_set('gui', 'vsize', vsize)
    self.config.default_set('gui', 'vsplit', self.ver_splitter.GetSashPosition())
    self.config.default_set('gui', 'hsplit', self.hor_splitter.GetSashPosition())
    self.config.default_set('gui', 'psplit', self.plot_splitter.GetSashPosition())
...
def on_grid_slider_check(self, event):
...
def plot_settings_changed(self, event):
    ...
    '''
    self.main_frame_toolbar.ToggleTool(10009, event.zoomstate)
...
def plot_page_changed(self, event):
...
def print_preview_plot(self, event):
...
def print_parameter_grid(self, event):
...
def print_preview_parameter_grid(self, event):
    ''' print_parameter_grid(frame, event) --> None
    
    Prints the table of parameters that have been fitted.
    '''
    self.paramter_grid.PrintPreview()

def copy_graph(self, event):
...
def change_data_grid_view(self, event):
...
def update_data_grid_choice(self, event):
...
def deactivate_cuda(self, event):
...
def data_loaders_help(self, event):
