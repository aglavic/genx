    ...
    if parameter=='sigma':
        ...
        for i in range(1, self.Repetitions):
            if self.sigma_gtype==0:
              ...
              par+=[refl.resolve_par(lay, parameter)*sqrt(1.+((sigma_gradient+lay.sigma_gradient)*i/(self.Repetitions-1))**2)
                    for lay in self.Layers]
              ...
              # add roughness using rms
              par+=[sqrt(refl.resolve_par(lay, parameter)**2+((sigma_gradient+lay.sigma_gradient)*i/(self.Repetitions-1))**2)
                    for lay in self.Layers]
            else:
        ...
        for i in range(self.Repetitions):
            par+=[refl.resolve_par(lay, parameter)*(1.-(d_gradient+lay.d_gradient)*(1./2.-float(i)/self.Repetitions)) for lay in self.Layers]
