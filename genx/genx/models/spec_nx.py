...
class Instrument(refl.ReflBase):
    ...
    Groups = [('General', ['wavelength', 'coords', 'I0', 'Ibkg', 'tthoff']),
...
def resolution_init(TwoThetaQz, instrument: Instrument):
    ...
    # Q vector given....
        ...
        # for tof the q-values are not angles but wavelenght, so tth-offset is a scaling factor
            ...
            Q = TwoThetaQz*(sin((ai+instrument.tthoff/2.)*pi/180.)/sin(ai*pi/180.))
        else:
            Q = TwoThetatoQ(instrument.wavelength,
                            QtoTheta(instrument.wavelength, TwoThetaQz)*2.0+instrument.tthoff)
    else:
