...
from . import grating, grating_diffuse
...
def test_calc_fields_single_interface():
    ...
    t, r, k_z = grating.calc_fields(
        qz / 2.0, 1.54, np.array([9e-5, 9e-5, 0], dtype=np.complex128), np.array([0.0, 0.0, 0.0])
    )
...
def test_calc_fields_single_layer():
    ...
    t, r, k_z = grating.calc_fields(qz / 2.0, 1.54, v, d)
    ...
    r_theory = (
        r1 * np.exp(1.0j * d[1] * k_z[:, 1]) / (t1 * t2) + r2 * np.exp(-1.0j * d[1] * k_z[:, 1]) / (t1 * t2)
    ) / (r1 * r2 * np.exp(1.0j * d[1] * k_z[:, 1]) / (t1 * t2) + np.exp(-1.0j * d[1] * k_z[:, 1]) / (t1 * t2))

    t_lay_theory = t2 * np.exp(1.0j * d[1] * k_z[:, 1]) / (r1 * r2 * np.exp(2 * 1.0j * d[1] * k_z[:, 1]) + 1.0)
    r_lay_theory = r1 * t2 * np.exp(1.0j * d[1] * k_z[:, 1]) / (r1 * r2 * np.exp(2 * 1.0j * d[1] * k_z[:, 1]) + 1.0)
