...
class TrapezoidBeam(Footprint):
    ...
    def __call__(self, theta, samplen):
        sinalpha = np.sin(theta*np.pi/180.)
        cross_section = samplen*sinalpha
        in_out_ratio = min(1.0, 2.*self.inner_width / (self.inner_width+self.outer_width))
        if self.inner_width>0:
            FP_in = np.minimum(1.0, cross_section/self.inner_width)
        else:
            FP_in = 0.0
        if self.outer_width>self.inner_width:
            FP_out = np.minimum(1.0, np.maximum(0.,
                                1.0-(1.0-np.minimum(1.0, (cross_section-self.inner_width)/(self.outer_width-self.inner_width)))**2))
        else:
            FP_out = 0.0
        return FP_in*in_out_ratio + FP_out*(1.-in_out_ratio)
