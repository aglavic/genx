...
from numpy import (abs, arange, arccos, arcsin, arctan, arctan2, array, c_, complex64, complex128, convolve, cos,
                   cumsum, dot, exp, float32, float64, hstack, imag, isscalar, linspace, loadtxt, log, log2, log10,
                   logical_not, maximum, minimum, nan, ndarray, newaxis, not_equal, ones, ones_like, pi, r_, real,
                   savetxt, sin, sqrt, sum, tan, transpose, where, zeros, zeros_like)

from .core.colors import CyclicList
from .core.custom_logging import iprint
from .core.h5_support import H5HintedExport, H5Savable
...
try:
    ...
    META_DEFAULT = {
        "data_source": {
            ...
            "sample": {"name": None},
            "measurement": {
                "instrument_settings": {"incident_angle": {"magnitude": None}, "wavelength": {"magnitude": None}},
                "data_files": [],
            },
        },
...
class DataSet(H5HintedExport):
    ...
    def __init__(self, name="", copy_from=None):
        ...
        self.plot_setting_names = ["color", "symbol", "symbolsize", "linetype", "linethickness"]
        if name != "":
            self.name = name
    ...
    def set_extra_data(self, name, value, command=None):
        ...
        """
        if name in ["x", "y", "e"]:
            raise KeyError("The extra data can not support the key" "names x, y or e.")
    ...
    def loadfile(self, filename, sep="\t", pos=0):
        ...
        try:
            ...
            if xcol < A.shape[1] and ycol < A.shape[1] and ecol < A.shape[1]:
                ...
                iprint(
                    "There are not enough columns in your data\n\
                       There are %i columns"
                    % A.shape[1]
                )
            return False
    ...
    def save_file(self, filename):
        ...
        """
        if self.x.shape == self.y_sim.shape and self.y.shape == self.error.shape and self.x.shape == self.y.shape:
            header = (
            ...
            debug = (
                "y_sim.shape: "
                + str(self.y_sim.shape)
                + "\ny.shape: "
                + str(self.y.shape)
                + "\nx.shape: "
                + str(self.x.shape)
                + "\nerror.shape: "
                + str(self.error.shape)
            )
            raise GenxIOError(
                "The data is not in the correct format all the" + "arrays have to have the same shape:\n" + debug,
                filename,
            )
    ...
    def try_commands(self, command_dict):
        ...
        if (xt.shape != yt.shape or xt.shape != et.shape or extra_shape) and result == "":
    ...
    def get_sim_plot_items(self):
        ...
        """
        return {
            "color": (self.sim_color[0] * 255, self.sim_color[1] * 255, self.sim_color[2] * 255),
    ...
    def get_data_plot_items(self):
        ...
        """
        return {
            "color": (self.data_color[0] * 255, self.data_color[1] * 255, self.data_color[2] * 255),
    ...
    def data_kwds(self):
        ...
        return dict(
            ...
            lw=self.data_linethickness,
            ls=self.data_linetype,
            marker=self.data_symbol,
            ms=self.data_symbolsize,
        )
    ...
    def sim_kwds(self):
        ...
        return dict(
            ...
            lw=self.sim_linethickness,
            ls=self.sim_linetype,
            marker=self.sim_symbol,
            ms=self.sim_symbolsize,
        )
    ...
    def _repr_ipyw_(self, add_header=True):
        ...
        if add_header:
            ...
            header = ipw.HBox(
                [
                    ...
                    for txt in [("Name", "30ex"), ("show", "6ex"), ("use", "6ex"), ("error", "6ex")]
                ]
            )
            vlist.append(header)
        ...
        for col in ["x", "y", "error"]:
            entr = ipw.Text(eval("self.%s_command" % col, globals(), locals()), description=col)
            entr._command = col
...
def c2html(colors):
    out = "#"
    for ci in colors:
        out += "%02x" % int(ci * 255.0)
    return out


def html2c(colors):
    out = (
        int("0x%s" % colors[1:3], 16) / 255.0,
        int("0x%s" % colors[3:5], 16) / 255.0,
        int("0x%s" % colors[5:7], 16) / 255.0,
    )
    return tuple(out)
...
class DataList(H5Savable):
    ...
    def set_simulated_data(self, sim_data):
        ...
        """
        [self.items[i].set_simulated_data(sim_data[i]) for i in range(self.get_len())]
    ...
    def set_fom_data(self, fom_data):
        ...
        flag use in the data.
        """
        [self.items[i].set_fom_data(fom_data[i]) for i in range(self.get_len())]
    ...
    def get_data_as_asciitable(self, indices=None):
        ...
        header1 = "".join(["%s\t\t\t\t" % self.items[index].name for index in indices])
        ...
        def exportFunc(index, row):
            ...
            if row < len(item.x):
                return "%e\t%e\t%e\t%e\t" % (item.x[row], item.y[row], item.error[row], item.y_sim[row])
            else:
                return " \t \t \t \t"

        # Now create the data
        text_data = "".join(["\n" + "".join([exportFunc(index, row) for index in indices]) for row in range(maxlen)])
        return header1 + "\n" + header2 + text_data
    ...
    def _repr_ipyw_(self):
        ...
        header = ipw.HBox(
            [
                ipw.HTML("<b>%s</b>" % txt[0], layout=ipw.Layout(width=txt[1]))
                for txt in [("No.", "6ex"), ("Name", "30ex"), ("show", "6ex"), ("use", "6ex"), ("error", "6ex")]
            ]
        )
        vlist.append(header)
        items = ["name", "show", "use", "use_error"]
