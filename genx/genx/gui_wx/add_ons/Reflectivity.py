...
class Plugin(framework.Template, SampleBuilder, wx.EvtHandler):
    ...
    def __init__(self, parent):
        ...
        self.mb_export_uncertainty = wx.MenuItem(
            menu, wx.NewId(), "Export Uncertainty...", "Export SLD uncertainty after a fit", wx.ITEM_NORMAL
        )
    ...
    def OnNewModel(self, event):
        ...
        dlg = wx.SingleChoiceDialog(
            self.parent, "Choose a model type to use", "Models", avail_models, wx.CHOICEDLG_STYLE
        )
    ...
    def CreateNewModel(self, modelname="models.spec_nx"):
        ...
        self.simulation_widget.SetSimArgs(
            ["Specular"] * nb_data_sets, ["inst"] * nb_data_sets, [["d.x"] for i in range(nb_data_sets)]
        )
        self.simulation_widget.UpdateListbox(update_script=True)
    ...
    def ReadModel(self, reevaluate=False, verbose=True):
        ...
        try:
            ...
            self.ShowErrorDialog(
                "Could not locate all data sets in the"
