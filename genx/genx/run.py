...
def extract_parameters(args):
    ...
    errors = [
        mod.parameters[index_names.index(ni)].error if mod.parameters.get_fit_state_by_name(ni) == 1 else "-"
        for ni in names
    ]
...
def start_fitting(args, rank=0):
    ...
    if rank == 0:
        ...
        iprint("Time to fit: ", (t2 - t1) / 60.0, " min")
...
def main():
    ...
    parser.add_argument(
        "infile", nargs="?", default="", help="The .gx or .hgx file to load or .ort file to use as basis for model"
    )
